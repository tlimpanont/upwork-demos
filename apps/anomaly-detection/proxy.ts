...
//      ENFORCE_DEMO_GATE=1.
//   2. NextAuth: only invoked for the paths anomaly-detection actually
//      protects (dashboard, projects, login, register). Mirrors the
//      previous narrow matcher so we don't pay a session lookup on
//      every page request. The actual decisions live in
//      lib/auth.config.ts `authorized()`.

const { auth } = NextAuth(authConfig);

const DEMO_ID = "anomaly-detection";
const DEMO_GATE_COOKIE = "demo_gate_passed";
const AUTH_PATHS = /^\/(dashboard|projects|login|register)(\/|$)/;
